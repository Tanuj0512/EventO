...
import { AdminMenu } from "../utils/menuWrap";
import { SubHeader } from "../Event _Dis/SubHeader";
...
const Admin=()=> {
  ...
  const fetchEventData = async () => {
    try {
      const eventsCollectionRef = query(collection(db, "event", ),where("Event_verified","==",""));
              ...
              <button className={styles.eventButton}>
                ...
                <div className={styles.sideButtons}>
                  {sessionStorage.getItem("type") === "admin" ? (
                    <AdminMenu eventId={event.eventId} />
                  ) :(<></>)}
                </div>
              </button>
            ))}
